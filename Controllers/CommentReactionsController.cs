...
public class CommentReactionsController : Controller
{
    ...
    {
        var identityDBContext = _context.CommentReactions.Include(c => c.Comment).Include(c => c.ReactionType)
            .Include(c => c.User);
    ...
    public async Task<IActionResult> Details(int? id)
    {
        ...
        if (commentReaction == null) return NotFound();
    ...
    public async Task<IActionResult> Create(
        [Bind("CommentReactionID,CommentID,UserID,ReactionTypeID")] CommentReaction commentReaction)
    {
        ...
        ViewData["CommentID"] =
            new SelectList(_context.Comments, "CommentID", "CommentText", commentReaction.CommentID);
        ViewData["ReactionTypeID"] = new SelectList(_context.ReactionTypes, "ReactionTypeID", "ReactionName",
            commentReaction.ReactionTypeID);
    ...
    public async Task<IActionResult> Edit(int? id,
        [Bind("CommentReactionID,CommentID,UserID,ReactionTypeID")] CommentReaction commentReaction)
    {
        if (id != commentReaction.CommentReactionID) return NotFound();
    ...
    {
        var commentReaction = await _context.CommentReactions.FindAsync(id);
        if (commentReaction != null) _context.CommentReactions.Remove(commentReaction);
    ...
    public async Task<IActionResult> CreateCommentReaction(
        [Bind("CommentID,UserID,ReactionTypeID")] CommentReaction newReaction)
    {
        if (string.IsNullOrEmpty(newReaction.UserID))
        {
            // Redirect the user to the login page
            var script = "<script>alert('You need to be logged in to perform this action.');</script>";
        ...
        var existingReaction = await _context.CommentReactions
            .FirstOrDefaultAsync(r =>
                r.CommentID == newReaction.CommentID && r.UserID == newReaction.UserID &&
                r.ReactionTypeID == newReaction.ReactionTypeID);
